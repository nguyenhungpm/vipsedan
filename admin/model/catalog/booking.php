...
class ModelCatalogBooking extends Model {
	public function addBooking($data) {
		...
		if (isset($data['booking_attribute'])) {
			foreach ($data['booking_attribute'] as $booking_attribute) {
				if ($booking_attribute['attribute_id']) {
					...
					foreach ($booking_attribute['booking_attribute_description'] as $language_id => $booking_attribute_description) {
						$this->db->query("DELETE FROM " . DB_PREFIX . "booking_attribute WHERE booking_id = '" . (int)$booking_id . "' AND attribute_id = '" . (int)$booking_attribute['attribute_id'] . "' AND language_id = '" . (int)$language_id . "'");
			...
			foreach ($data['booking_option'] as $booking_option) {
				if ($booking_option['type'] == 'select' || $booking_option['type'] == 'radio' || $booking_option['type'] == 'checkbox' || $booking_option['type'] == 'image') {
					if (isset($booking_option['booking_option_value'])) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "booking_option SET booking_id = '" . (int)$booking_id . "', option_id = '" . (int)$booking_option['option_id'] . "', required = '" . (int)$booking_option['required'] . "'");

						$booking_option_id = $this->db->getLastId();

						foreach ($booking_option['booking_option_value'] as $booking_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "booking_option_value SET booking_option_id = '" . (int)$booking_option_id . "', booking_id = '" . (int)$booking_id . "', option_id = '" . (int)$booking_option['option_id'] . "', option_value_id = '" . (int)$booking_option_value['option_value_id'] . "', quantity = '" . (int)$booking_option_value['quantity'] . "', subtract = '" . (int)$booking_option_value['subtract'] . "', price = '" . (float)$booking_option_value['price'] . "', price_prefix = '" . $this->db->escape($booking_option_value['price_prefix']) . "', points = '" . (int)$booking_option_value['points'] . "', points_prefix = '" . $this->db->escape($booking_option_value['points_prefix']) . "', weight = '" . (float)$booking_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($booking_option_value['weight_prefix']) . "'");
						}
					}
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "booking_option SET booking_id = '" . (int)$booking_id . "', option_id = '" . (int)$booking_option['option_id'] . "', value = '" . $this->db->escape($booking_option['value']) . "', required = '" . (int)$booking_option['required'] . "'");
				}
			}
		}

		if (isset($data['booking_recurring'])) {
			foreach ($data['booking_recurring'] as $recurring) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "booking_recurring` SET `booking_id` = " . (int)$booking_id . ", customer_group_id = " . (int)$recurring['customer_group_id'] . ", `recurring_id` = " . (int)$recurring['recurring_id']);
			}
		}
		...
		if (isset($data['booking_reward'])) {
			foreach ($data['booking_reward'] as $customer_group_id => $booking_reward) {
				if ((int)$booking_reward['points'] > 0) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "booking_reward SET booking_id = '" . (int)$booking_id . "', customer_group_id = '" . (int)$customer_group_id . "', points = '" . (int)$booking_reward['points'] . "'");
				}
			}
		}
